...
## Plan: ATHENA — Alpha M&E Platform Systems Specification

**TL;DR:** A self-hosted, single-organisation M&E platform consisting of three tightly integrated components: a native Android offline-first data-collection app, an Express.js/PostgreSQL backend API, and a SvelteKit management dashboard. The specification below covers all subsystems, interfaces, data contracts, and quality controls required for the Alpha release.

---

### Phase 1 — System Overview & Constraints
...
**Relevant files:**
- Draft-Project-Specification.md — primary architecture source
- db.question.md — schema tier breakdown

---

### Phase 2 — Data Architecture (PostgreSQL)
...
**Relevant files:**
- sample.schema.md — reference DDL
- data.storage.md — JSONB storage model with JSON examples
- folder.clarification.md — Folder = PostgreSQL Schema
- reporting.md — views, GIN indexes, Materialized Views roadmap
...
13. Define API module boundaries:
    - `AuthModule` — OAuth 2.0 / SSO (Google, Microsoft via Passport.js or `openid-client`)
    ...
    - `QualityModule` — automated pipeline at ingestion (freshness, uniqueness checks)
    - `IndicatorsModule` — ITT CRUD and aggregation queries
    - `ReportingModule` — serves flattened view data to SvelteKit dashboard
...
### Phase 4 — Android Data Collection App
...
### Phase 5 — Management Dashboard (SvelteKit)
...
26. Specify OAuth SSO integration on the SvelteKit side: redirect flow to Google/Microsoft, receive JWT session token from Express after verification
27. Specify real-time quarantine alerts: Server-Sent Events (SSE) or WebSocket channel from Express → SvelteKit for live quarantine queue updates
...
34. Specify Docker Compose services: `postgres` (PostGIS image), `api` (Node.js), `web` (Node.js serving SvelteKit build)
...
| Draft-Project-Specification.md | Primary architecture source |
| sample.schema.md | Reference DDL |
| data.storage.md | JSONB storage model |
| folder.clarification.md | Folder = PostgreSQL Schema rationale |
| reporting.md | Views, GIN indexes, chart libraries |
| db.question.md | Schema tiers and ITT model |
...
- OAuth only (no local password auth) → no `password_hash` column needed on `users` table (remove from sample schema)
